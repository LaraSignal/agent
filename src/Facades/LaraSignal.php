 ...
 * @method static mixed storage(string $operation, string $disk, string $path, \Closure $callback, array<string, mixed> $attributes = [])
 * @method static void runtime(string $name, array<string, mixed> $attributes = [], string $status = 'completed')
 * @method static void heartbeat(string $name = 'scheduler', int $expectedEverySeconds = 60, array<string, mixed> $attributes = [])
 * @method static void broadcastMetric(string $provider, int $connections, ?int $messages = null, array<string, mixed> $attributes = [])
